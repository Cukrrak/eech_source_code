...
{
	select_blackhawk_target_acquisition_system (TARGET_ACQUISITION_SYSTEM_FLIR);
}
...
{
	select_blackhawk_target_acquisition_system (TARGET_ACQUISITION_SYSTEM_DTV);
}
...
{
	inc_blackhawk_eo_zoom();
}

static void dec_eo_zoom_event (event *ev)
{
	dec_blackhawk_eo_zoom();
}
...
{
	centre_blackhawk_target_acquisition_system ();
}
...
{
	toggle_blackhawk_show_allied_targets ();
}
...
{
	deactivate_blackhawk_master_caution ();
}
...
static void virtual_cockpit_track_target_event (event *ev)
{
	...
	if ((ev->key == DIK_1) || (get_view_mode () != VIEW_MODE_VIRTUAL_COCKPIT_TRACK_TARGET))
	{
		...
		{
			set_blackhawk_lock_target (TRUE);
		}
	}
	else
	{
...
static void select_next_tsd_ase_range_event (event *ev)
{
	...
	{
		select_next_blackhawk_pilot_tsd_ase_range ();
	}
	else
	{
		select_next_blackhawk_co_pilot_tsd_ase_range ();
	}
}
...
static void select_previous_tsd_ase_range_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_previous_blackhawk_pilot_tsd_ase_range ();
	}
	else
	{
		select_previous_blackhawk_co_pilot_tsd_ase_range ();
	}
}
...
static void select_next_tsd_declutter_level_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_next_blackhawk_pilot_tsd_declutter_level ();
	}
	else
	{
		select_next_blackhawk_co_pilot_tsd_declutter_level ();
	}
}
...
static void select_previous_tsd_declutter_level_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_previous_blackhawk_pilot_tsd_declutter_level ();
	}
	else
	{
		select_previous_blackhawk_co_pilot_tsd_declutter_level ();
	}
}
...
{
	toggle_blackhawk_ase_auto_page ();
}
...
static void select_next_main_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
{
		select_next_blackhawk_mfd (BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
}
	else
{
...
static void select_next_top_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
{
...
static void select_next_bottom_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
{
...
static void select_previous_main_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
{
		select_previous_blackhawk_mfd (BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
	}
	else
{
		select_previous_blackhawk_mfd (BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
	}
}
...
static void select_main_tads_mfd_event (event *ev)
{
	if (eo_sensor == TARGET_ACQUISITION_SYSTEM_FLIR)
{
		if (get_crew_role () == CREW_ROLE_PILOT)
{
			select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_FLIR, BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
		}
		else
	{
			select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_FLIR, BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
	}
	}
	else
	{
		if (get_crew_role () == CREW_ROLE_PILOT)
	{
			select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_DTV, BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
	}
	else
	{
			select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_DTV, BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
		}
	}
	}
...
static void select_main_tsd_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_TSD, BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
	}
	else
	{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_TSD, BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
	}
	}
...
static void select_main_ase_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
...
static void select_main_weapon_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_WEAPON, BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
	}
	else
{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_WEAPON, BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
	}
}
...
static void select_main_system_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_SYSTEM, BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
	}
	else
	{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_SYSTEM, BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
	}
}
...
static void select_main_engine_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
...
static void select_main_mission_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
{
		select_blackhawk_mfd_mode (BLACKHAWK_MFD_MODE_MISSION, BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
}
	else
{
...
static void toggle_main_mfd_on_off_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
{
		toggle_blackhawk_mfd_on_off (BLACKHAWK_MFD_LOCATION_PILOT_MAIN);
}
	else
{
		toggle_blackhawk_mfd_on_off (BLACKHAWK_MFD_LOCATION_CO_PILOT_MAIN);
	}
}
...
static void toggle_top_mfd_on_off_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
...
static void toggle_bottom_mfd_on_off_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
...
{
	toggle_blackhawk_infra_red_jammer_manual ();
}
...
static void activate_laser_event(event* ev)
			{
	if (!laser_is_active() && !blackhawk_damage.laser_designator)
...
void set_blackhawk_avionics_events (void)
{
...
//	set_event (DIK_NUMPADENTER, MODIFIER_LEFT_ALT, KEY_STATE_DOWN, toggle_lock_target_event); // Jabberwock 031107 Designated targets
	...
	set_event (DIK_LBRACKET, MODIFIER_NONE, KEY_STATE_DOWN, select_next_main_mfd_event);
	set_event (DIK_RBRACKET, MODIFIER_NONE, KEY_STATE_DOWN, select_previous_main_mfd_event);
	set_event (DIK_LBRACKET, MODIFIER_LEFT_CONTROL, KEY_STATE_DOWN, toggle_main_mfd_on_off_event);

	set_event (DIK_LBRACKET, MODIFIER_RIGHT_CONTROL, KEY_STATE_DOWN, select_next_top_mfd_event);
	set_event (DIK_RBRACKET, MODIFIER_RIGHT_CONTROL, KEY_STATE_DOWN, select_next_bottom_mfd_event);
	set_event (DIK_LBRACKET, MODIFIER_LEFT_ALT, KEY_STATE_DOWN, toggle_top_mfd_on_off_event);
